...
class FilmMakinesiProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val items = document.select("div.film-box, article.film, div.dizi-box, div.movie-card, div.tv-card")
            .mapNotNull { it.toSearchResult() }
        return newHomePageResponse(request, items, hasNext = true)
    }

    private fun Element.toSearchResult(): SearchResponse? {
        val title = this.selectFirst("h2 a, h3 a, h4 a, .title a, .film-title, .dizi-title")?.text()?.trim() ?: return null
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return document.select("div.film-box, article.film, div.dizi-box, div.movie-card, div.tv-card")
            .mapNotNull { it.toSearchResult() }
    }
